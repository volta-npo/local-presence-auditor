...
### A zero-login audit that shows a local business exactly where its online footprint is missing, broken, or invisible.
...
**Digital Presence** · **Repo 01/50** · **No backend. No login. Client data stays local.**

[Live app](https://volta-npo.github.io/01-local-presence-auditor/) · [Report an issue](https://github.com/volta-npo/01-local-presence-auditor/issues) · [Volta](https://voltanpo.org)
...
**Local Presence Auditor** is a polished, local-first open-source tool from Volta's 50-project OSS division. It helps Volta student pods auditing restaurants, service businesses, and nonprofits before kickoff turn real community work into structured evidence, client-safe handoffs, and mentor-reviewable release packets.
...
SEO suites are built for marketers with budgets. Small businesses need a humane, evidence-based first audit that can be run during a 30-minute discovery call.

### The niche

Under-resourced local businesses with low or fragmented digital presence.

### North-star metric

`time from first call to prioritized roadmap`
...
flowchart TD
    ...
    Evidence --> Workbench["Presence Audit Console"]
    Workbench --> Score["Presence readiness"]
    Score --> Cert["v3 production certification"]
    Cert --> A1["Owner-safe audit report"]
    Cert --> A2["Impact-effort roadmap"]
    Cert --> A3["CSV findings ledger"]
...
flowchart LR
    subgraph Modules["First-class modules"]
        M1["Website basics scanner"]
        M2["Maps/listing evidence checklist"]
        M3["Review and social proof snapshot"]
        M4["Action plan exporter"]
    end
    M1 --> Workbench
    M2 --> Workbench
    M3 --> Workbench
...
sequenceDiagram
    ...
    participant T as Local Presence Auditor
...
|---|---|
| **Domain workbench** | A purpose-built audit interface for a zero-login audit that shows a local business exactly where its online footprint is missing, broken, or invisible. |
...
|---:|---|---|
| 1 | **Website basics scanner** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 2 | **Maps/listing evidence checklist** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 3 | **Review and social proof snapshot** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 4 | **Action plan exporter** | Converts field work into repeatable, reviewable Volta delivery evidence. |
...
```bash
git clone https://github.com/volta-npo/01-local-presence-auditor.git
cd 01-local-presence-auditor
npm test
npm start
```
